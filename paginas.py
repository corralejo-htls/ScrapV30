...
# ============================================================
# Alcance real del pipeline: 20 idiomas base + "it" (italiano), incorporado
# DESPUÉS, AL FINAL del proceso => 21 idiomas en total (Finding 6 resuelto).
# (zh, ja, ko, th) son CJK y (ar) es RTL. La extracción primaria se apoya en
# JSON-LD y en atributos (data-testid / data-atlas-latlng), independientes del
...
def get_sql_languages():
    ...
    por defecto, lenguajes_activos. Conserva el ORDEN de lenguajes_activos."""
...
# Mapeo OPCIONAL de código de idioma -> locale de la BD (PASO 3).
...
# ============================================================
# --- FORMATO: SIN FORMATO. Los textos del DAT se guardan TAL CUAL se extraen.
# ============================================================
# REGLA (requisito): los textos/datos se guardan EXACTAMENTE como aparecen en el
# scraping; SIN formato alguno. Se ELIMINA todo el formato que el código añadía
# (antes se convertían los saltos de línea a "<br />"). Por tanto el PASO 2 (DAT)
# NO transforma el texto. Cualquier limpieza/normalización se hace DESPUÉS, en el
# PASO 3 (al armar el SQL), sin modificar el DAT en disco.

_BR_RE = re.compile(r'<br\s*/?>', re.IGNORECASE)

def strip_all_formatting(text):
    """
    Quita TODO el formato de un texto (saltos de línea y <br /> heredados de
    DAT antiguos) y colapsa los espacios. Resultado: una sola línea, sin formato.
    """
    if not text:
        return ""
    text = _BR_RE.sub(' ', text)                 # compat: <br /> de DAT antiguos
    text = text.replace('\r\n', ' ').replace('\r', ' ').replace('\n', ' ')
    text = re.sub(r'\s+', ' ', text).strip()     # colapsa todo el espacio en blanco
    return text
...
# ============================================================
# --- NUEVO PROCESO DE LIMPIEZA (PASO 2.5): entre DAT y SQL
# ============================================================
# Se aplica DESPUÉS del PASO 2 (DAT) y ANTES del PASO 3 (SQL). Lee el texto CRUDO
# del DAT y produce un texto limpio para el SQL. NO modifica el DAT en disco.
#
# Limpia la contaminación de interfaz/atribución que Booking adosa a los textos:
#   - "© OpenStreetMap" (atribución de distancias; constante en todos los idiomas)
#   - frases de valoración de ubicación ("... rated it 9.7", "location score of 9.7",
#     "Top location:", "particularly like", "score from N reviews", "X/10!")
#   - coletillas de interfaz ("Real guests • Real stays", "show map")
# La contaminación se localiza por idioma; el marcador universal y fiable es
# "OpenStreetMap". El resto son heurísticas (marcadores EN + patrones numéricos)
# aplicadas SÓLO como salvaguarda; son conservadoras para no recortar contenido.

# Marcador universal (presente en todos los idiomas).
_CONTAM_UNIVERSAL = ("openstreetmap",)
# Marcadores estructurales / EN (salvaguarda; ampliable por idioma).
_CONTAM_MARKERS = (
    "real guests", "show map", "particularly like", "location score of",
    "top location", "rated by recent guests",
)
# Patrones de línea que delatan widgets de valoración (independiente de idioma).
_CONTAM_PATTERNS = (
    re.compile(r'\b\d+[.,]\d+\s*/\s*10\b'),          # "9.7/10"
    re.compile(r'\brated it\s+\d', re.IGNORECASE),   # "rated it 9.7"
    re.compile(r'score from\s+[\d.,]+\s*\w*review', re.IGNORECASE),  # "(score from 5625 reviews)"
)

def _is_contaminated_line(line):
    low = line.strip().lower()
    if not low:
        return False
    if any(m in low for m in _CONTAM_UNIVERSAL):
        return True
    if any(m in low for m in _CONTAM_MARKERS):
        return True
    if any(p.search(line) for p in _CONTAM_PATTERNS):
        return True
    return False
...
def extract_hotel_data(page, lang):
    ...
    # La limpieza de contaminación de interfaz/atribución se aplica en el
    # PASO 3 (clean_text_for_sql), entre el DAT y el SQL.
...
# extraen del scraping, SIN formato (no se añade <br />, no se normaliza). La
# exclusión de medios/URLs y la limpieza de contaminación se realizan DESPUÉS,
# en el PASO 3 (clean_text_for_sql). Así el DAT es el reflejo fiel del DOM y no
# se modifica salvo nueva adquisición.
...
def generate_sql_updates(dat_structures):
    ...
    Aplica el NUEVO PROCESO DE LIMPIEZA (clean_text_for_sql) al texto CRUDO del
    DAT ANTES de construir el SQL (PASO 2.5: entre DAT y SQL). No modifica el DAT.
    ...
    Mejoras:
      ...
      - OMITE traducciones que son FALLBACK en inglés (idénticas al inglés),
        para no escribir inglés como traducción (Finding 1).
    ...
    for dat in dat_structures:
        ...
        # PASO 2.5 — limpieza del texto CRUDO del DAT antes del SQL.
        default_desc = clean_text_for_sql(hotels.get(default_lang, {}).get("hotel_description", ""))
        # Referencia inglesa para detectar fallbacks (idéntico => no es traducción).
        en_desc_clean = clean_text_for_sql(hotels.get("en", {}).get("hotel_description", ""))
