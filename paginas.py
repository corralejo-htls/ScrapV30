...
Instalación:
    ...
    -> en CMD instalar :
     ...
     -> en CMD ejecutar (usos): 
            python paginas.py
            Python paginas.py --solo-sql --ids=75888,75889,75891,75892,7589975901
            python paginas.py --CSV
...
Cambios v5.8 (limpieza de código y simplificación de configuración):
    ...
    - [M1+] Derivación de CIUDAD GENERALIZADA: derive_city contempla varios
      órdenes de dirección (la ciudad puede ir ANTES o DESPUÉS del CP). Austria
      "1010 Vienna" y Chequia "Prague, 110 00" -> Vienna/Prague (y localizados:
      Wien/Prag, ウィーン/プラハ...). Como fuente SECUNDARIA, si el JSON-LD no da
      ciudad se intenta con la 1ª línea de address_full (DOM). `region` vacío en
      origen (p. ej. Praga) se deja vacío: NO se infiere (no se inventan datos).
    ...
    - [PASO 1] REGLA: el idioma 'en' SIEMPRE se scrapea, aunque NO esté en
      lenguajes_activos (es el idioma de referencia/default y base para detectar
      fallbacks). Implementado en get_scrape_languages().
    - [MODO --CSV] Nueva columna calculada 'Translate_EN': TRUE si el
      hotel_description del idioma es IGUAL al del idioma por defecto ('en')
      -> probable fallback (candidato a traducción); FALSE si difiere (traducido);
      VACÍO si lang == 'en' (la ficha ya está en inglés). Se calcula en una
      PRE-PASADA sobre el texto CRUDO del DAT. El CSV NO aplica Format_DAT ni
      ninguna otra modificación: su objetivo es el análisis de los datos crudos.
    - [STEALTH] Compatibilidad con playwright-stealth 2.x (clase Stealth,
      apply_stealth_sync) y 1.x (stealth_sync). Antes el import asumía la API 1.x
      y, con la 2.x instalada, fallaba y caía al stealth manual. Ahora se detecta
      la API disponible (_stealth_apply) y el arranque indica cuál se usa.
    - [Format_DAT] Limpieza/formateo que se aplica SÓLO al generar SQL (PASO 3),
      sobre una copia: NO modifica el DAT en disco ni el CSV. Bandera maestra
      FORMAT_DAT_ENABLED (True predeterminado; cualquier otro valor cancela todo).
      Reglas: hotel_description -> quitar contaminación de widget (marcadores
      universales OpenStreetMap/Genius) + normalizar espacios (quitar espacios al
      final de línea y recortar extremos) + reemplazar \\n por "<br />";
      info_importante -> normalizar espacios; address_full -> reconstruir desde el
      JSON-LD (street_address). Salvaguarda anti-sobrelimpieza.
    - [MODO --CSV] Exportación de TODOS los DAT de 'data' a un único CSV
      estructurado tipo tabla (Excel) con cabecera: una fila por (id, idioma) y
      TODOS los campos. Uso: python paginas.py --CSV (independiente de PASO 1/2/3,
      NO genera SQL, NO admite ningún otro parámetro). Se guarda en 'data' como
      CSV_<mmdd>_.csv y se sobrescribe sin confirmación. utf-8-sig para Excel.
...
lenguajes = {
    ...
    "pt": "pt.html?lang=pt",
    "us": "html?lang=en-us",
    "el": "el.html?lang=el",  # griego 
    "pl": "pl.html?lang=pl",  # polonia    
    "ru": "ru.html?lang=ru",  # ruso
    "ar": "ar.html?lang=ar",  # árabe 
    "nl": "nl.html?lang=nl",  # neerlandés 
    "tl": "tl.html?lang=tl",  # tagalo/Filipinas 
    "id": "id.html?lang=id",  # indonesio
    "ms": "ms.html?lang=ms",  # Malayo 
    "th": "th.html?lang=th",  # tailandés 
    "hu": "hu.html?lang=hu",  # húngaro
    "no": "no.html?lang=no",  # noruego 
    "fi": "fi.html?lang=fi",  # finlandés
    "sv": "sv.html?lang=sv",  # sueco
    "da": "da.html?lang=da",  # danés
    "hi": "hi.html?lang=hi",  # hindi
    "ja": "ja.html?lang=ja",  # japonés
    "ko": "ko.html?lang=ko",  # coreano
    "tw": "zh-tw.html?lang=zh-tw",  #  chino Taiwán
    "zh": "zh-cn.html?lang=zh-cn",  #  chino 
}
...
# BANDERA MAESTRA de Format_DAT (limpieza/formateo SOLO al generar SQL):
#   True (predeterminado) -> se aplica Format_DAT al armar los archivos SQL.
#   CUALQUIER OTRO VALOR  -> se cancelan TODAS las operaciones de Format_DAT
#                            (passthrough: el SQL se arma con el texto tal cual).
# Format_DAT NO modifica el DAT en disco ni el CSV; sólo la vista usada para SQL.
FORMAT_DAT_ENABLED = True
...
# --- MODO --CSV: exportación de TODOS los DAT a un CSV (independiente) ---
# python paginas.py --CSV  -> exporta todos los DAT de la carpeta 'data' a un
# único CSV estructurado tipo tabla (Excel), con cabecera, en la misma carpeta.
# NO está asociado al PASO 1/2/3, NO genera SQL y NO admite NINGÚN otro
# parámetro (ni --solo-sql, ni --ids=, etc.).
CSV_ONLY = any(a in ("--CSV", "--csv") for a in sys.argv[1:])
if CSV_ONLY:
    _otros = [a for a in sys.argv[1:] if a not in ("--CSV", "--csv")]
    if _otros:
        print(f"[CSV] --CSV no es compatible con ningún otro parámetro: {_otros}. "
              f"Uso: python paginas.py --CSV")
        sys.exit(2)
...
# --- Idiomas a SCRAPEAR (PASO 1) ---
def get_scrape_languages():
    """PASO 1 — idiomas a scrapear. REGLA: el idioma 'en' SIEMPRE se incluye,
    aunque NO esté en lenguajes_activos, porque es el idioma de referencia
    (default_language) y la base para detectar si una traducción es fallback.
    Conserva el orden de lenguajes_activos; si 'en' falta, se antepone."""
    langs = list(lenguajes_activos)
    if "en" not in langs:
        langs = ["en"] + langs
    return langs
...
# Stealth (opcional) — compatible con playwright-stealth 2.x (clase Stealth) y
# 1.x (función stealth_sync). _stealth_apply(page) aplica el stealth según la API
# disponible; STEALTH_AVAILABLE indica si se pudo cargar alguna.
_STEALTH_API = None
try:
    # API nueva (>=2.0): clase Stealth con apply_stealth_sync(page|context)
    from playwright_stealth import Stealth as _Stealth
    _stealth_raw = _Stealth().apply_stealth_sync
    _STEALTH_API = "2.x"
except Exception:
    try:
        # API antigua (1.x): función stealth_sync(page)
        from playwright_stealth import stealth_sync as _stealth_raw
        _STEALTH_API = "1.x"
    except Exception:
        _stealth_raw = None

STEALTH_AVAILABLE = _stealth_raw is not None

def _stealth_apply(page):
    """Aplica el stealth de la librería (no-op si no está disponible)."""
    if _stealth_raw is not None:
        _stealth_raw(page)
...
if not SQL_ONLY and not CSV_ONLY and not is_admin():
...
if not PLAYWRIGHT_AVAILABLE and not SQL_ONLY and not CSV_ONLY:
...
if PLAYWRIGHT_AVAILABLE:
    ...
    if STEALTH_AVAILABLE:
        print(f"[OK] playwright-stealth disponible (API {_STEALTH_API})")
    else:
        print("[INFO] playwright-stealth NO disponible. Usando stealth manual.")
...
def load_urls_from_csv(csv_path="list_url.csv", max_urls=MAX_URLS_DOWNLOAD):
    ...
    _scrape_langs = get_scrape_languages()
    print(f"[OK] Idiomas activos: {lenguajes_activos}")
    if "en" not in lenguajes_activos:
        print("[OK] REGLA PASO 1: 'en' se scrapea SIEMPRE (no está en lenguajes_activos).")
    print(f"[OK] Scrapes totales a realizar: {len(entries) * len(_scrape_langs)}")
    return entries
...
# ============================================================
def derive_city(street_address, postal_code, country="", locality=""):
    """
    Devuelve la CIUDAD a partir de la dirección completa (streetAddress),
    contemplando DISTINTOS órdenes de dirección (la ciudad puede ir ANTES o
    DESPUÉS del código postal).

    Motivo (M1 + generalización): en Booking el JSON-LD `addressLocality` suele
    contener la CALLE, no la ciudad. La ciudad real aparece dentro de
    `streetAddress`, junto al código postal, pero el ORDEN varía según el país:
        Austria:  "<calle>, <distrito>, <CP> <Ciudad>, <País>"   ("1010 Vienna")
        Chequia:  "<calle>, <Ciudad>, <CP>, <País>"              ("Prague, 110 00")
    El código postal sirve de ancla (independiente del idioma). Algoritmo:
      1) se retira el país del final (si coincide con addressCountry);
      2) si un segmento es SÓLO el CP -> la ciudad es el segmento adyacente;
      3) si un segmento empieza/termina por el CP -> la ciudad es el resto;
      4) en último caso, el último segmento que no sea la calle.
    Nunca devuelve la calle ni el propio CP.

    NO es limpieza de contenido: es un mapeo estructurado de metadatos. NO
    inventa datos: devuelve "" si no puede determinarla con fiabilidad.
    """
    sa = str(street_address or "").strip()
    pc = str(postal_code or "").strip()
    if not sa:
        return ""
    segs = [s.strip() for s in sa.split(",") if s.strip()]
    ctry = str(country or "").strip()
    if ctry and segs and segs[-1].casefold() == ctry.casefold():
        segs = segs[:-1]   # retirar el país para no confundirlo con la ciudad
    if not segs:
        return ""
    street = str(locality or "").strip()   # addressLocality (suele ser la calle)

    def _ok(c):
        c = (c or "").strip(" ,").strip()
        # nunca la calle, ni vacío, ni el propio CP
        if not c or c == pc or (street and c == street):
            return ""
        return c

    if pc:
        # (a) segmento que es SÓLO el CP -> la ciudad es el segmento adyacente
            ...
            if seg == pc:
                for j in (i - 1, i + 1):
                    if 0 <= j < len(segs):
                        ...
                        if c:
                            return c
        # (b) segmento que empieza/termina por el CP -> la ciudad es el resto
        for seg in segs:
            if seg.startswith(pc) or seg.endswith(pc):
                c = _ok(seg.replace(pc, " "))
                if c:
                    return c
    # (c) heurística: último segmento que no sea la calle (país ya retirado)
...
def extract_hotel_data(page, lang):
    ...
    # CIUDAD (M1 generalizado): se deriva de `streetAddress` contemplando que la
    # ciudad vaya antes o después del CP (Austria: "1010 Vienna"; Chequia:
    # "Prague, 110 00"). derive_city ya descarta la calle y nunca inventa datos.
    city = derive_city(addr.get("streetAddress", ""), addr.get("postalCode", ""),
                       country=data["country"], locality=data["locality"])
    # Enriquecimiento (opcional): si el JSON-LD no da ciudad, se intenta con la
    # 1ª línea del bloque de dirección del DOM (address_full) como FUENTE
    # SECUNDARIA. Sigue sin inventar datos (misma lógica de derivación).
    if not city and data["address_full"]:
        af_line = data["address_full"].split("\n", 1)[0]
        city = derive_city(af_line, addr.get("postalCode", ""),
                           country=data["country"], locality=data["locality"])
    data["city"] = city
    # NOTA (region): si `addressRegion` viene vacío en el origen (p. ej. Praga),
    # se deja vacío. NO se infiere una región inexistente: forzarla sería
    # inventar datos y rompería la fidelidad del DAT.
...
# ============================================================
# --- Format_DAT — limpieza/formateo SOLO para la generación de SQL
# ============================================================
# Se ejecuta ÚNICAMENTE al generar los archivos SQL (PASO 3) y se aplica sobre
# una COPIA del DAT: NO modifica el DAT en disco, ni el CSV, ni el scraping.
# Gobernado por FORMAT_DAT_ENABLED (True = activo; cualquier otro valor = se
# cancelan todas las operaciones y el texto pasa tal cual). Determinista.

# Cadena que sustituye a los saltos de línea \n EN hotel_description.
# (configurable; estándar HTML de salto de línea).
HOTEL_DESC_BR = "<br />"

# Marcadores UNIVERSALES de contaminación de widget de ubicación/valoración.
# La eliminación es POR LÍNEAS (se descarta la línea que contiene el marcador),
# sin distinción de mayúsculas/minúsculas.
FORMAT_CONTAM_MARKERS = ("openstreetmap", "genius")

def _fmt_norm_ws(text):
    """Normalización de espacios: quita los espacios al FINAL de cada línea y
    recorta los extremos. NO colapsa saltos de línea internos (fiel al texto)."""
    if not text:
        return ""
    t = str(text).replace("\r\n", "\n").replace("\r", "\n")
    t = "\n".join(ln.rstrip() for ln in t.split("\n"))   # espacios al final de línea
    return t.strip()                                     # recortar extremos

def _fmt_strip_widget(text):
    """Elimina la contaminación del widget de ubicación/valoración quitando las
    líneas que contienen un marcador universal (OpenStreetMap / Genius)."""
    if not text:
        return ""
    out = [ln for ln in str(text).split("\n")
           if not any(m in ln.lower() for m in FORMAT_CONTAM_MARKERS)]
    return "\n".join(out)

def _fmt_desc_br(text):
    """SÓLO hotel_description: reemplaza los saltos de línea \\n por HOTEL_DESC_BR."""
    if not text:
        return ""
    return str(text).replace("\n", HOTEL_DESC_BR)

def _fmt_rebuild_address_full(text, rec=None):
    """Reconstruye address_full desde el JSON-LD estructurado. En Booking,
    `street_address` YA es la dirección completa y limpia, por lo que se usa
    directamente; si falta, se compone desde las partes disponibles (de-dup).
    No inventa datos: si no hay nada, devuelve la 1ª línea del valor original."""
    rec = rec or {}
    sa = (rec.get("street_address") or "").strip()
    if sa:
        return sa
    partes = [rec.get("locality", ""), rec.get("postal_code", ""),
              rec.get("city", ""), rec.get("region", ""), rec.get("country", "")]
...
# Reglas por campo (orden de aplicación). Ampliable sin tocar el motor.
#   hotel_description: quitar contaminación -> normalizar espacios -> \n a </br>
#   info_importante  : normalizar espacios
#   address_full     : reconstruir desde JSON-LD
FORMAT_RULES = {
    "hotel_description": ["strip_widget", "norm_ws", "desc_br"],
    "info_importante":   ["norm_ws"],
    "address_full":      ["rebuild_addr"],
}
_FORMAT_HELPERS = {
    "norm_ws": _fmt_norm_ws,
    "strip_widget": _fmt_strip_widget,
    "desc_br": _fmt_desc_br,
    "rebuild_addr": _fmt_rebuild_address_full,
}

def format_dat(dat, log=print):
    """
    Devuelve una COPIA del DAT con los campos formateados según FORMAT_RULES,
    para usarla EXCLUSIVAMENTE al generar SQL. NO modifica el DAT en disco.

    Si FORMAT_DAT_ENABLED no es True, se CANCELAN todas las operaciones y se
    devuelve el DAT sin cambios (passthrough). Conservador: si una regla dejara
    vacío un campo no vacío, se AVISA y se conserva el valor original.
    """
    if FORMAT_DAT_ENABLED is not True:
        return dat   # cualquier valor distinto de True cancela Format_DAT
    out = copy.deepcopy(dat)
    tocados = 0
    for lang, rec in out.get("hotels", {}).items():
        for field, rules in FORMAT_RULES.items():
            ...
            value = original
            for rule in rules:
                fn = _FORMAT_HELPERS.get(rule)
                if not fn:
                    log(f"[FORMAT] [WARN] regla desconocida: {rule}")
                    continue
                try:
                    value = fn(value, rec) if rule == "rebuild_addr" else fn(value)
                except Exception as e:
                    log(f"[FORMAT] [WARN] {field}/{lang} {rule}: {str(e)[:80]}")
            # salvaguarda anti-sobrelimpieza
            if str(original).strip() and not str(value).strip():
                log(f"[FORMAT] [WARN] {field}/{lang}: la limpieza dejó vacío -> se conserva el original")
                value = original
            if value != original:
                tocados += 1
            rec[field] = value
    log(f"[FORMAT] Format_DAT aplicado ({tocados} campos formateados; DAT en disco intacto).")
    return out
...
def generate_sql_updates(dat_structures):
    ...
    El texto se procesa con Format_DAT (si FORMAT_DAT_ENABLED is True) SOLO para
    el SQL; el DAT en disco no se modifica. Procesa en lotes de hasta
    MAX_SQL_BATCH. Garantiza consistencia ID <-> DAT <-> SQL.
    Nombre de archivo: SQL__<MMDD>_<lote>_.sql
    ...
    for dat in dat_structures:
        # Format_DAT: limpieza/formateo SOLO para el SQL (no toca el DAT en disco).
        dat = format_dat(dat)
        hotel_id = dat.get("id", "")
        ...
        # Texto ya formateado por Format_DAT (sólo para el SQL).
...
# ============================================================
# --- EXPORTACIÓN A CSV (MODO --CSV, independiente de PASO 1/2/3)
# ============================================================
def export_dat_to_csv(dat_dir=DAT_DIR):
    """
    MODO --CSV: exporta TODOS los DAT de `dat_dir` a un ÚNICO archivo CSV
    estructurado tipo tabla (Excel), con cabecera, en la MISMA carpeta `dat_dir`.

    - Una FILA por (id de hotel, idioma); los metadatos del hotel (id, name,
      scraped_at, ...) se repiten en cada fila.
    - La PRIMERA fila es la cabecera con los nombres de columna.
    - Incluye TODOS los campos: los de nivel hotel (menos 'hotels') + 'lang' +
      todos los campos por idioma (DATA_FIELDS y cualquier extra encontrado).
    - Nombre: CSV_<mmdd>_.csv  (mmdd = mes+día). Se SOBRESCRIBE sin confirmación.
    - utf-8-sig (Excel abre bien acentos/CJK/RTL) y comillado CSV estándar
      (los campos con comas, comillas o saltos de línea se entrecomillan).

    NO genera SQL ni ningún otro artefacto. Devuelve la ruta del CSV o None.
    """
    dats = load_dat_files_from_disk(dat_dir)   # lee todos los DAT__*.json
    if not dats:
        print("[CSV] No hay DAT que exportar.")
        return None

    # Columnas de nivel hotel (orden preferido + cualquier extra encontrado)
    top_cols = ["id", "name", "scraped_at", "generated_at", "url", "default_language"]
    for d in dats:
        for k in d.keys():
            if k != "hotels" and k not in top_cols:
                top_cols.append(k)
    # Columnas por idioma (DATA_FIELDS + cualquier extra encontrado)
    lang_cols = list(DATA_FIELDS)
    for d in dats:
        for rec in d.get("hotels", {}).values():
            for k in rec.keys():
                if k not in lang_cols:
                    lang_cols.append(k)
    # Columna CALCULADA (no es un campo del DAT): valida si hotel_description es
    # IGUAL al del idioma por defecto ("en"). TRUE => mismo texto que el inglés
    # (probable fallback -> candidato a traducción); FALSE => ya traducido.
    # Para lang == "en" se deja VACÍO (la ficha ya está en inglés: no aplica).
    EXTRA_COL = "Translate_EN"
    header = top_cols + ["lang"] + lang_cols + [EXTRA_COL]

    # === PRE-PASADA: se calcula Translate_EN AQUÍ, sobre el texto CRUDO del DAT.
    # El CSV NO aplica Format_DAT ni ninguna otra modificación de datos: su objetivo
    # es el análisis de los datos CRUDOS. Por eso la comparación es sobre el texto
    # tal cual del DAT (sin \n -> "<br />", sin normalización, etc.). ===
    desc_eq = {}   # (id, lang) -> "TRUE"/"FALSE"/"" (vacío para "en")
    for d in dats:
        hid = str(d.get("id", ""))
        hotels = d.get("hotels", {})
        def_lang = d.get("default_language") or "en"
        ref_desc = ((hotels.get(def_lang) or hotels.get("en") or {})
                    .get("hotel_description", "") or "").strip()
        for lang, rec in hotels.items():
            if lang == "en":
                desc_eq[(hid, lang)] = ""   # ficha en inglés -> campo vacío
                continue
            this_desc = (rec.get("hotel_description", "") or "").strip()
            desc_eq[(hid, lang)] = "TRUE" if (this_desc and this_desc == ref_desc) else "FALSE"

    mmdd = datetime.now().strftime("%m%d")
    out_path = os.path.join(dat_dir, f"CSV_{mmdd}_.csv")

    filas = 0
    with open(out_path, "w", newline="", encoding="utf-8-sig") as f:
        w = csv.writer(f)          # dialecto 'excel': coma, comillado, CRLF
        w.writerow(header)
        for d in dats:
            hid = str(d.get("id", ""))
            top_vals = ["" if d.get(c) is None else d.get(c) for c in top_cols]
            hotels = d.get("hotels", {})
            # idiomas en el orden de lenguajes_activos y luego el resto
            orden = [l for l in lenguajes_activos if l in hotels] + \
                    [l for l in hotels if l not in lenguajes_activos]
            for lang in orden:
                rec = hotels.get(lang, {})
                lang_vals = ["" if rec.get(c) is None else rec.get(c) for c in lang_cols]
                # valor pre-calculado sobre el texto crudo (ver pre-pasada arriba)
                w.writerow(top_vals + [lang] + lang_vals + [desc_eq.get((hid, lang), "FALSE")])
                filas += 1
    print(f"[CSV] Exportados {len(dats)} DAT ({filas} filas, {len(header)} columnas) -> {out_path}")
    return out_path
        ...
        try:
            ...
            if STEALTH_AVAILABLE:
                try:
                    ...
                    print(f"  -> [WARN] stealth no aplicado: {e}")
...
# === MODO --CSV: exportar TODOS los DAT a un CSV (independiente de PASO 1/2/3) ===
# No scrapea, no construye DAT y no genera SQL. Sólo lee los DAT de 'data' y
# escribe un CSV estructurado tipo tabla. No admite ningún otro parámetro.
if CSV_ONLY:
    print("[MODE] --CSV: exportación de los DAT a CSV (sin PASO 1/2/3, sin SQL).")
    export_dat_to_csv(DAT_DIR)
    print("Proceso (solo CSV) finalizado.")
    sys.exit(0)
...
try:
    with sync_playwright() as p:
        ...
        for idx, (hotel_id, url_base) in enumerate(url_entries):
            ...
            for lang in get_scrape_languages():
