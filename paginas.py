...
# ============================================================
# Alcance del pipeline: lista de idiomas activos (configurable según necesidad).
# El número y la composición pueden variar; `lenguajes` (más abajo) es el
# CATÁLOGO de plantillas de URL del que se seleccionan los activos.
...
# Idioma por defecto del sistema de destino (PASO 3, tabla `hotel`). Ya NO se lee
# del CSV: el CSV sólo contiene (id, url). De momento la tabla `hotel` usa 'en'
# (language = 'en'); podría variar en futuras actualizaciones del sistema destino.
DEFAULT_LANG = "en"
...
# extrae. NO se elimina ni transforma nada (ni contaminación de interfaz, ni
# medios/URLs, ni saltos de línea). La limpieza se hará en una actualización
# posterior (proceso "Format_DAT"), entre el PASO 2 (DAT) y el PASO 3 (SQL), sin
# modificar el DAT. (No implementado en esta revisión.)
